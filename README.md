...
Local-First Architecture: Engineered for local inference (via LM Studio/Ollama) to prioritize student privacy and enable accessibility in low-connectivity global environments.
...
Project Structure
generation_factory.py: The multi-agent orchestration engine.

socratic_judge.py: The evaluation script implementing LLM-as-a-Judge.

report_generator.py: A utility for aggregating audit results into performance metrics.

data/: Directory for JSONL datasets and audit logs.
